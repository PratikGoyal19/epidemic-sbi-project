...
Outputs:
- theta: shape (n_samples, 3), columns [beta, gamma, I0]
...
def sample_prior(
    ...
    Returns theta of shape (n_samples, 3): [beta, gamma, I0]
    """
    beta  = rng.uniform(beta_min,  beta_max,  size=n_samples)
    gamma = rng.uniform(gamma_min, gamma_max, size=n_samples)
    i0    = rng.uniform(i0_min,    i0_max,    size=n_samples)
    return np.column_stack([beta, gamma, i0])
...
def print_statistics(theta: np.ndarray, x: np.ndarray, N: int) -> None:
    ...
    print(f"  I0:    min={theta[:, 2].min():.2f},  max={theta[:, 2].max():.2f},  mean={theta[:, 2].mean():.2f}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--beta-max",   type=float, default=0.60)
    parser.add_argument("--gamma-min",  type=float, default=0.01)
    parser.add_argument("--gamma-max",  type=float, default=0.10)
    parser.add_argument("--i0-min",     type=float, default=1.0)
    parser.add_argument("--i0-max",     type=float, default=50.0)
...
def main() -> None:
    ...
    print("="*70)
    print(" GENERATING SIR TRAINING DATASET (beta, gamma, I0)")
    print("="*70)
    ...
    print(f"I0 range:    [{args.i0_min}, {args.i0_max}]")
