...
Metrics: MAE, RMSE, Coverage, Posterior Recovery Plots, SBC
Parameters: beta, gamma, I0
...
PARAM_NAMES  = ["β (beta)", "γ (gamma)", "I₀ (I0)"]
PARAM_LABELS = ["beta", "gamma", "i0"]
N_PARAMS     = 3
...
def get_nle_posterior_samples(
    ...
    """
    Importance-weighted sampling from NLE posterior using uniform prior.
    x: (n_test, 160) → returns (n_test, n_samples, 3)
    """
    rng    = np.random.default_rng(seed)
    n_test = x.shape[0]
    ...
    # Prior bounds — must match generate_data.py
    beta_min,  beta_max  = 0.10, 0.60
    gamma_min, gamma_max = 0.01, 0.10
    i0_min,    i0_max    = 1.0,  50.0

    all_samples = np.zeros((n_test, n_samples, 3), dtype=np.float32)
    ...
    for i in range(n_test):
        ...
        while len(collected) < n_samples:
            ...
            log_liks = amortizer.log_likelihood({
                "observables": x_rep,
...
def compute_mae(theta_true: np.ndarray, posterior_samples: np.ndarray) -> dict:
    """theta_true: (n_test, 3), posterior_samples: (n_test, n_samples, 3)"""
    ...
    mae_i0    = float(np.mean(np.abs(posterior_mean[:, 2] - theta_true[:, 2])))
    mae_mean  = float((mae_beta + mae_gamma + mae_i0) / 3)
    return {"mae_beta": mae_beta, "mae_gamma": mae_gamma, "mae_i0": mae_i0, "mae_mean": mae_mean}
...
def compute_rmse(theta_true: np.ndarray, posterior_samples: np.ndarray) -> dict:
    ...
    rmse_i0    = float(np.sqrt(np.mean((posterior_mean[:, 2] - theta_true[:, 2])**2)))
    return {"rmse_beta": rmse_beta, "rmse_gamma": rmse_gamma, "rmse_i0": rmse_i0}
...
def compute_coverage(theta_true: np.ndarray, posterior_samples: np.ndarray,
    ...
    for level in levels:
        alpha = (1 - level) / 2
        lower = np.quantile(posterior_samples, alpha,     axis=1)  # (n_test, 3)
        upper = np.quantile(posterior_samples, 1 - alpha, axis=1)

        in_interval = (theta_true >= lower) & (theta_true <= upper)
        results[f"coverage_{int(level*100)}"] = {
            "beta":  float(in_interval[:, 0].mean()),
            "gamma": float(in_interval[:, 1].mean()),
            "i0":    float(in_interval[:, 2].mean()),
            "mean":  float(in_interval.mean()),
        }
    return results
...
def plot_npe_vs_nle_comparison(theta_true, npe_samples, nle_samples, out_dir):
    methods     = ["NPE", "NLE"]
...
def plot_metrics_summary(npe_metrics: dict, nle_metrics: dict, out_dir: Path) -> None:
    ...
    # MAE per parameter
    for k, (label, key) in enumerate(zip(["β (beta)", "γ (gamma)", "I₀"], ["mae_beta", "mae_gamma", "mae_i0"])):
        ax   = axes[k]
        vals = [npe_metrics["mae"][key], nle_metrics["mae"][key]]
        bars = ax.bar(methods, vals, color=colors, alpha=0.8, edgecolor="black")
        ax.set_title(f"MAE — {label}", fontsize=12)
...
def main():
    ...
    print(f"  theta shape: {theta_true.shape}  [beta, gamma, I0]")
    ...
    print(f"{'MAE I0':<35} {npe_mae['mae_i0']:>12.4f} {nle_mae['mae_i0']:>12.4f}")
    ...
    print(f"{'RMSE I0':<35} {npe_rmse['rmse_i0']:>12.4f} {nle_rmse['rmse_i0']:>12.4f}")
    ...
    print(f"{'50% Coverage I0':<35} {npe_coverage['coverage_50']['i0']:>12.3f} {nle_coverage['coverage_50']['i0']:>12.3f}")
    ...
    print(f"{'90% Coverage I0':<35} {npe_coverage['coverage_90']['i0']:>12.3f} {nle_coverage['coverage_90']['i0']:>12.3f}")
    print("=" * 70)
